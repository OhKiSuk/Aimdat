...
class CorpId(models.Model):
    ...
    corp_sectors_main = models.CharField(max_length=255, null=True) #소속 섹터(대분류)
    corp_sectors = models.CharField(max_length=255, null=True) #소속 섹터(예: 제조업, 서비스업 등)
    base_date = models.DateField(null=True) # 정보 기준일
    is_financial_industry = models.BooleanField(default=False) #금융, 비금융 구분(FALSE: 비금융, TRUE: 금융)
