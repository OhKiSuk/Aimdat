...
from ..views.google_login_views import GoogleLinkOffView
...
class GoogleLoginLinkOffTest(TestCase):
    ...
    @patch('requests.post')
    @patch.object(GoogleLinkOffView, 'linkoff')
    def test_delete_account_success_if_google_login_user(self, mock_linkoff, mock_post):
        """
        구글 소셜 로그인 사용자가 사용자 계정 삭제 시도를 할 경우 성공
        """
        mock_post.return_value.text = '{"access_token": "test_token"}'
        mock_linkoff.return_value = 200

        response = self.client.get(reverse('account:google_login_linkoff'))
        self.assertEqual(response.status_code, 302)
        self.assertFalse(User.objects.filter(email='testuser@aimdat.com').exists())

    @patch('requests.post')
    @patch.object(GoogleLinkOffView, 'linkoff')
    def test_delete_account_failure_if_google_login_user(self, mock_linkoff, mock_post):
        """
        구글 소셜 로그인 사용자의 연동 해제에 실패했을 경우
        """
        mock_post.return_value.text = '{"access_token": "test_token"}'
        mock_linkoff.return_value = 500
